...
const StationsList = ({ loading }) => {
  ...
  const dynamicFiltered = stations.filter((station) => {
    ...
    const tags = typeof station.genres === 'string'
      ? station.genres.split(',').map((tag) => tag.trim().toLowerCase())
      : [];

    const genreMatch = tags.includes(term);

    return nameMatch || countryMatch || genreMatch;
  });
 ...
 const searchBy_ = (event) => {
  ...
  const sortByTrend = (event) => {
    ...
    const sorted = [...filtered].sort((a, b) => b.clicktrend - a.clicktrend);
  ...
  if (stations.length === 0) {
    return selectedGenre ? (
      ...
      <div className="main-content">
        <h2>Browse genres to find radio stations</h2>
        <p>
          <img src={no_stations}></img>
        </p>
      </div>
  ...
  return (
    <div className="main-content">
      <div className="stations-list">
        ...
        <button onClick={sortByClicks}>
          Sort by {isAscending ? 'least' : 'most'} popular all time
        </button>
        ...
        <ul>
          {filtered.map((station) => (
            <li key={station.stationuuid}>
              ...
              <h3>
                <img
                  src={station.image_url ? station.image_url : no_stations}
                ></img>
              </h3>
              <h3>{station.country}</h3>

              {user && (
                <button onClick={() => addFavorites(user, station)}>
                  Save
                </button>
              )}
